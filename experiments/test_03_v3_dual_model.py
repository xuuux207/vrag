"""
实验3 v3 双模型版本：8B总结 + 14B回复

测试目标：
1. 验证使用更小模型是否能显著降低TTFT
2. 对比四种方法：直接RAG、批量总结+RAG、渐进式总结v2+RAG、渐进式总结v3+增量RAG
3. 评估双模型架构的延迟和质量权衡

配置：
- Summary LLM: 服务器本地 vLLM (Qwen3-8B) - localhost:8000
- Response LLM: 服务器本地 vLLM (Qwen3-14B) - localhost:8001
...
# 双模型配置
SUMMARY_BASE_URL = "http://localhost:8000/v1"
SUMMARY_MODEL = "Qwen/Qwen3-8B"

RESPONSE_BASE_URL = "http://localhost:8001/v1"
RESPONSE_MODEL = "Qwen/Qwen3-14B"

# 评估模型配置 - 使用Qwen API的大模型
EVAL_BASE_URL = os.getenv("QWEN_API_BASE", "https://dashscope.aliyuncs.com/compatible-mode/v1")
EVAL_MODEL = "qwen3-max"  # 使用大模型进行评估
EVAL_TOKEN = os.getenv("QWEN_TOKEN")
...
"""
    ...
    response = eval_client.chat.completions.create(
        ...
        temperature=0.3,
        max_tokens=1000,
        extra_body={"enable_thinking": False}  # 禁用Qwen思考标签
    )
...
"""
    ...
    gen_start = time.time()
    final_response, ttft, gen_time, token_count = generate_response_streaming(prompt, response_client)
...
def method3_incremental_summary(
    ...
    """方法3：边输入边总结，最后进行RAG"""
    # 1. 渐进式总结（使用8B模型）
    summarizer = SimpleSummarizer(summary_client, model_name=SUMMARY_MODEL)
    segment_results = []
...
def method4_incremental_rag(
    ...
    summarizer = IncrementalRAGSummarizer(
        ...
        relevance_threshold=0.3  # 降低阈值，允许更多相关文档
    )
    segment_results = []
...
class Experiment3DualModelRunner:
    """实验3 双模型运行器（8B总结 + 14B回复）"""

    def __init__(self):
        print("\n初始化实验3 双模型版本...")
        print(f"Summary LLM: {SUMMARY_MODEL} @ {SUMMARY_BASE_URL}")
        print(f"Response LLM: {RESPONSE_MODEL} @ {RESPONSE_BASE_URL}")
        print(f"Eval LLM: {EVAL_MODEL} @ {EVAL_BASE_URL}")
            ...
            for method_name, future in futures.items():
                try:
                    ...
                    if method_name == "method1":
                        ...
                        print(f"  ✓ 方法1完成（{result_data['timing']['total_time']:.2f}秒, TTFT={result_data['timing']['ttft']:.2f}秒）")
                    elif method_name == "method2":
                        result["method2_batch"] = result_data
                        print(f"  ✓ 方法2完成（{result_data['timing']['total_time']:.2f}秒, TTFT={result_data['timing']['ttft']:.2f}秒）")
                    elif method_name == "method3":
                        result["method3_incremental"] = result_data
                        print(f"  ✓ 方法3完成（输入后: {result_data['timing']['total_time_after_input']:.2f}秒, TTFT={result_data['timing']['ttft']:.2f}秒）")
                    else:
                        result["method4_incremental_rag"] = result_data
                        print(f"  ✓ 方法4完成（输入后: {result_data['timing']['total_time_after_input']:.2f}秒, TTFT={result_data['timing']['ttft']:.2f}秒, 检索{result_data['metrics']['total_relevant_docs']}个文档）")
    ...
    def run_all_tests(self):
        ...
        print(f"  2. Batch Summary - 等输入完成后总结+RAG (8B)")
        print(f"  3. Incremental v2 - 边输入边总结，最后RAG (8B)")
        print(f"  4. Incremental v3 (新) - 边输入边总结+增量RAG，相关度过滤 (8B)\n")
        ...
        output_file = output_dir / f"experiment3_dual_model_results_{timestamp}.json"
...
def main():
    runner = Experiment3DualModelRunner()
